#!/usr/local/bin/bash
# This file contains the install script for testbw

# Initialise defaults
JAIL_NAME="testbw"
DB_DATABASE=${JAIL_NAME}
DB_USER=${JAIL_NAME}
INSTALL_TYPE=${testbw_type}
DB_HOST="$(sed 's|\(.*\)/.*|\1|' <<<"${mariadb_ip4_addr}"):3306"
DB_PASSWORD="${testbw_db_password}"
DB_STRING="mysql://${DB_USER}:${DB_PASSWORD}@${DB_HOST}/${DB_DATABASE}"
ADMIN_TOKEN=${testbw_admin_token}
...
iocage exec testbw "curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y"
